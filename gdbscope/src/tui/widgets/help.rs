...
pub fn draw(f: &mut Frame, area: Rect, view: &ViewState) {
    ...
    let t = vec![
        ...
        Line::from(""),
        h("[3] Locals"),
        Line::from("  w  p  m  y     Watch / eval / memory / ptype (prefilled)"),
        ...
        Line::from(""),
        h("[9] Watch"),
        Line::from("  w  d  p  m  y  Add / remove / eval / memory / ptype"),
        Line::from(""),
        h("[0] Output"),
        Line::from("  :              Raw GDB command"),
        Line::from("  ;              Repeat last command"),
        Line::from("  :walk <expr> [as T]           Walk std::list → Explorer"),
        Line::from("  :walk from <a> to <b> as T    Walk raw pointer chain"),
        Line::from(""),
        h("Inspection (auto-prefills from context)"),
        Line::from("  X              Variable inspector (actions popup)"),
        ...
        Line::from("  y              Ptype (show type definition, prefills from context)"),
        Line::from(""),
        h("[I] Explorer"),
        Line::from("  I              Add to explorer (prefills from Locals/Watch/Source)"),
        Line::from("  Enter          Expand / collapse node"),
        Line::from("  d              Remove root and subtree"),
        Line::from("  y  p  m        Ptype / eval / memory (prefilled from selection)"),
        d("  Values auto-refresh on stop. Changed values glow red"),
        Line::from(""),
        h("Analysis"),
        ...
        d("  Ruby/Java/Node detected -> runtime command hints"),
        d("  All analysis keys work: x T f s S L P a"),
        Line::from(""),
        h("Python (CPython)"),
        Line::from("  Y              Toggle Python <-> native (C) view"),
        d("  Auto-enters Python view when python-gdb.py helpers load"),
        d("  Stack/Source/Locals show Python-level frames in PY mode"),
        d("  Enter on a Stack frame runs py-up/py-down"),
        d("  Needs helpers: set auto-load safe-path / (if py-* undefined)"),
        Line::from(""),
        h("General"),
